...
const getBookById = async function (req, res) {
    try {
        ...
        let bookIdRegex = /^[a-f\d]{24}$/
        if (!bookId.match(bookIdRegex)) {
            return res.status(400).send({ status: false, msg: "bookId (ObjectId) Must be 24 byte" })
        }
...
const deleteBookById = async function (req, res) {
    try {
        ...
        let bookIdRegex = /^[a-f\d]{24}$/
        if (!bookId.match(bookIdRegex)) {
            return res.status(400).send({ status: false, msg: "bookId (Object)Id Must be 24 byte" })
        }
        let bookData = await bookModels.findOne({ _id: bookId })
        if (!bookData) return res.status(404).send({ status: false, message: "No data found" })
        if (bookData.isDeleted == false) {
            let deleteData = await bookModels.findOneAndUpdate({ _id: bookId }, { $set: { isDeleted: true, deletedAt: Date.now() } }, { new: true })
        } else {
...
module.exports = { createBooks, getBookById, deleteBookById }